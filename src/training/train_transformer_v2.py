...
# Глобальные дефолты (переопределяются из CLI в __main__)
FOCAL_GAMMA = 2.0
LABEL_SMOOTH = 0.05
TAG_OVERRIDE = ""
...
def main(config_path, model_name, smoke, gamma, smooth, max_len_ov,
         epochs_ov, lr_ov):
    global FOCAL_GAMMA, LABEL_SMOOTH, TAG_OVERRIDE
    FOCAL_GAMMA = gamma
    LABEL_SMOOTH = smooth
    ...
    max_len = max_len_ov or cfg["model"]["max_length"]
    if epochs_ov:
        cfg["train"]["epochs"] = epochs_ov
    if lr_ov:
        cfg["train"]["lr"] = lr_ov
    ...
    class WTrainer(Trainer):
        """Focal loss (gamma) + class weights — для дисбаланса и редких
        классов (Ремонт/Засор тянут macro-F1 вниз). gamma=0 => weighted CE."""
    ...
    tag = TAG_OVERRIDE or model_name.split("/")[-1]
    log_experiment(registry, {
        ...
        "notes": f"fine-tune {'SMOKE' if smoke else 'full'}, "
                 ...
                 f"focal_g={FOCAL_GAMMA}, smooth={LABEL_SMOOTH}, v2"})
...
if __name__ == "__main__":
    ...
    ap.add_argument("--gamma", type=float, default=2.0,
                    help="focal loss gamma (0 = weighted CE)")
    ap.add_argument("--smooth", type=float, default=0.05,
                    help="label smoothing")
    ap.add_argument("--max_len", type=int, default=0,
                    help="override max_length (0 = из конфига); 256 для длинных")
    ap.add_argument("--epochs", type=int, default=0,
                    help="override epochs (0 = из конфига); 6-8 рекомендуется")
    ap.add_argument("--lr", type=float, default=0.0,
                    help="override lr (0 = из конфига)")
    ap.add_argument("--tag", default="",
                    help="имя модели в реестре (для offline-путей)")
    a = ap.parse_args()
    TAG_OVERRIDE = a.tag
    main(a.config, a.model, a.smoke, a.gamma, a.smooth, a.max_len,
         a.epochs, a.lr)
